// Build assets/solar-grid.json: monthly mean daily solar exposure (MJ/m²/day)
// averaged from Bureau of Meteorology AWAP daily grids for a calendar year.
// Source: https://www.bom.gov.au/jsp/awap/solar/ (product: solarave daily grids)
// Usage: node scripts/fetch-solar.mjs [year] [cacheDir]
...
const CACHE = process.argv[3] || join(fileURLToPath(import.meta.url), '..', '.solar-cache');
const OUT = join(fileURLToPath(import.meta.url), '..', '..', 'assets', 'solar-grid.json');
...
const url = (m, d) => {
  ...
  return `https://www.bom.gov.au/web03/ncc/www/awap/solar/solarave/daily/grid/0.05/history/nat/${s}${s}.grid.Z`;
};
...
writeFileSync(OUT, JSON.stringify({
  src: `BOM AWAP daily solar exposure grids, averaged over ${YEAR}; MJ/m²/day`,
