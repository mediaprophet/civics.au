// Build assets/tmax-grid.json: monthly mean daily MAXIMUM temperature (°C)
...
// Usage: node scripts/fetch-maxtemp.mjs [year] [cacheDir]
...
const CACHE = process.argv[3] || join(fileURLToPath(import.meta.url), '..', '.tmax-cache');
const OUT = join(fileURLToPath(import.meta.url), '..', '..', 'assets', 'tmax-grid.json');
...
const url = (m, d) => {
  ...
  return `https://www.bom.gov.au/web03/ncc/www/awap/temperature/maxave/daily/grid/0.05/history/nat/${s}${s}.grid.Z`;
};
...
writeFileSync(OUT, JSON.stringify({
  src: `BOM AWAP daily maximum temperature grids, averaged over ${YEAR}; °C`,
